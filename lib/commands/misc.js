(async () => {
     switch(true) {
         case /^(menu|help)$/i.test(command): {
             ...
             conn.sendMessage(from, { contentText: menu(m, body, pushname).trim(), footerText: '©YT hy fauzi', buttons: [{ buttonId: `${prefix}owner`, buttonText: { displayText: 'Owner' }, type: 1 },{ buttonId: `${prefix}tos`, buttonText: { displayText: 'Cara Penggunaan' }, type: 1 }], headerType: 'IMAGE', imageMessage: prep }, 'buttonsMessage', { quoted: m })
             break
         }
         ...
         case /^owner$/i.test(command): {
             ...
             for (let i of global.ownerNumber.map(v => v.replace(/\D/g, '') + '@s.whatsapp.net')) {
                 listOwner.push({ vcard: `BEGIN:VCARD\nVERSION:3.0\nN:;;;;\nFN:${conn.getName(i)}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Home\nitem2.URL:https://youtube.com/channel/UC-iDjlzju2qTWjoyHmlrG_Q\nitem2.X-ABLabel:YT HY FAUZI\nitem3.X-ABLabel:REALZI BOT\nEND:VCARD` })
             }
